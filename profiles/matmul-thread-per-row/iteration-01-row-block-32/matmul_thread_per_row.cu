...
void matmul(const float *M_h, const float *N_h, float *out_h, size_t row_M, size_t col_M, size_t col_N) {
  const size_t row_threads_per_block = 32;
  ...
  printf("Kernel average time over %d launches: %.3f ms\n", timing_iterations, elapsed_ms / timing_iterations);

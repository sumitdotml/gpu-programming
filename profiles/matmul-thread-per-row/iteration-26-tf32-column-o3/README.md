# Iteration 26: TF32 column mapping at O3

## 1. One change

Changed only the compiler optimization level from `-O2` to `-O3` for the two-warp adjacent-column mapping.
...
| **Median** | **0.232** | **0.743** |
| Mean | 0.233 | 0.743 |
| Sample standard deviation | 0.002 | 0.002 |
...
modal shell --no-pty --gpu A10G --image nvidia/cuda:12.8.1-devel-ubuntu24.04 \
  --add-local profiles/matmul-thread-per-row/iteration-26-tf32-column-o3 \
  -c 'nvcc -O3 -arch=sm_86 /mnt/iteration-26-tf32-column-o3/matmul_thread_per_row.cu -I/mnt/iteration-26-tf32-column-o3 -o /tmp/matmul.out && for run in 1 2 3 4 5; do echo "RUN=$run"; /tmp/matmul.out; done'
```

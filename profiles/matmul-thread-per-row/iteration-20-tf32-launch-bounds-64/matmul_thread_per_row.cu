...
__global__ __launch_bounds__(64) void MatMulKernel(const float *M_d, const float *N_d, float *out_d, size_t row_M, size_t col_M, size_t col_N) {
  constexpr int tile_size = 16;
  constexpr int k_tile_size = 8;
  const size_t warp = threadIdx.x / 32;
  const size_t row = (blockIdx.y * 2 + warp) * tile_size;
  const size_t col = blockIdx.x * tile_size;
...
void matmul(const float *M_h, const float *N_h, float *out_h, size_t row_M, size_t col_M, size_t col_N) {
  ...
  const dim3 dimBlock(64, 1, 1);
  const dim3 dimGrid((col_N + tile_size - 1) / tile_size, (row_M + 2 * tile_size - 1) / (2 * tile_size), 1);

...
__global__ void MatMulKernel(const float *M_d, const float *N_d, float *out_d, size_t row_M, size_t col_M, size_t col_N) {
  ...
  for (size_t tile_start = 0; tile_start < col_M; tile_start += tile_size) {
    const size_t M_col = tile_start + threadIdx.x;
    const size_t N_row = tile_start + threadIdx.y;
    M_tile[threadIdx.y][threadIdx.x] = row < row_M && M_col < col_M ? M_d[row * col_M + M_col] : 0.0f;
    N_tile[threadIdx.y][threadIdx.x] = N_row < col_M && col < col_N ? N_d[N_row * col_N + col] : 0.0f;
    __syncthreads();
    for (int k = 0; k < tile_size; ++k)
      accumulator += M_tile[threadIdx.y][k] * N_tile[k][threadIdx.x];
    __syncthreads();
  }
  if (row < row_M && col < col_N)
    out_d[row * col_N + col] = accumulator;
}
...
void matmul(const float *M_h, const float *N_h, float *out_h, size_t row_M, size_t col_M, size_t col_N) {
  ...
  const dim3 dimGrid((col_N + threads_per_dim - 1) / threads_per_dim, (row_M + threads_per_dim - 1) / threads_per_dim, 1);
